...
test("manifest records the verified latest host and keeps alpha unclaimed", () => {
  assert.equal(manifest.dsh.compatibility.dshReleases["0.1.5-rc.2"], "compatible");
...
test("latest host seams are pinned for repeatable development tests", () => {
  for (const name of [
    ...
    assert.match(manifest.peerDependencies[name], /\^0\.1\.5-rc\.2/);
  }
});
